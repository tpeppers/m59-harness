...
async function build({ chunk = 300 } = {}) {
  ...
  for (const r of rooms) {
    ...
    // KOD ROOM COORDINATES ARE 1-BASED, AND THESE ARE KOD COORDINATES. LEAVE THEM ALONE.
    //
    // `rows: 24` means rows 1..24, so a door at row 24 is the LAST ROW — not one past the
    // end. barlport.kod:47 declaring [29, 27] in a 29-row room is a door on the far wall,
    // correctly stated, and The Crypt's only exit is [24, 9] in a 24-row room.
    //
    // I clamped these once, on 2026-09-07, having compared 1-based coordinates against a
    // 0-based bound and concluded 83 doors were off the map. That moved The Crypt's only
    // door one square and sealed a character in for hours; the operator walked to the door
    // in the client and reported it at 24,9, exactly where the .kod puts it.
    //
    // THE REAL PROBLEM IS THAT ONE FIELD IS READ IN TWO SPACES. Movement (`look`,
    // `walk_to`, the anchors the router walks to) is 1-based like the .kod. `RoomGeometry`
    // is 0-based, because it indexes a flat array. Whatever hands a goExit to the geometry
    // must subtract one; the goExit itself must not be edited to suit it. See
    // m59-routebake.mjs, which does the conversion at the point of use.
