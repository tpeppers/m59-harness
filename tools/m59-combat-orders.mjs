...
export function parseCombatCommand(line) {
  ...
  if (!['attack', 'ambush', 'stop', 'status'].includes(action))
  ...
  const target = ['attack', 'ambush'].includes(action) ? unquote(words.shift() ?? '') : undefined;
  const options = {};
  for (const word of words) {
    const m = /^(agents|map|at|door|radius|ttl|order_id)=(.+)$/.exec(word);
  ...
  if (!agents?.length) throw new Error('combat needs agents=t1,t2');
  const allowed = new Set(action === 'ambush' ? ['agents', 'map', 'at', 'door', 'radius', 'ttl']
    : action === 'attack' ? ['agents', 'ttl'] : action === 'stop' ? ['agents', 'order_id'] : ['agents']);
  ...
  if (action === 'stop' || action === 'status')
    return { agents, order: { action, ...(options.order_id ? { order_id: options.order_id } : {}) } };
  ...
  return { agents, order: normalizeCombatOrder(order) };
}

export async function dispatchCombatOrders({ agents, order, send }) {
