...
function state() {
  ...
  return {
    ...
    items: c?.inventory ? c.inventory.map(o => ({
      ...
      tag: o.tag ?? null,
      flags: o.flags ?? 0,
      // GRAFTED FROM ec6e907 (on origin/main). A wand reads as the bare word "wand" until
      // identified and every Wand subclass shares one icon, so the palette translation is
      // the only thing that tells a wand of identification from a Qor wand. Carried raw.
