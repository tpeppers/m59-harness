...
import { sayToNpc } from './m59-sayrange.mjs';
...
async function runStep(ctx, agent, step, state) {
  ...
  switch (step.do) {
    ...
    case 'say': {
      return sayToNpc(step, {
        look: () => call('look', { agent }, 30_000).catch(() => null),
        walkTo: (target, opts) => call('walk_to',
          { agent, col: target.col, row: target.row, arrive_within: opts.arriveWithin }, opts.timeoutMs),
        log: text => ctx.log(agent, text),
        speak: async text => {
          ...
          return { spoken_ok: !spoken?.error, replies };
        },
      });
    }
