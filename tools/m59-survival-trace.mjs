...
function context(s, t, now) {
  ...
  return {
    ...
    posture_command: pick(c?.lastPostureCommand,['verb','at','room_object_id']),
    position_reads: {requested:c?.roomContentsRequested??null,received:c?.roomContentsReceived??null,
      lost:c?.roomContentsLost??0},
    ...
    hold: traceRefuge(k?.hold), frozen: { until: k?.frozenUntil ?? null,
      froze_at:k?.frozeAt??null,without_gain:k?.freezesWithoutGain??null,
