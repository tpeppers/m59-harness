...
- `m59-deskpractice-test.mjs` (70): practising spells at the service desk. The mana reserve is `reserve_casts` (2) x one cast of the dearest service the desk both OFFERS and the caster KNOWS, priced from the kod catalogue (60 on a desk with reveal, 24 without); the desk's reagents are kept back on top of the Rescue emeralds; a cast landing exactly on the reserve is allowed and one under is not; a cast that spends no mana sets the spell aside; and the keeper never practises away from its post, under an errand, or with a desk job in flight — and `passErrand` serves the desk first.
